...
The abcquality.org export is already flat (one row per provider, no
multi-value or nested cells), so SC needs only a few state-specific transforms:

  1. RATING. `ABC Level` is a letter grade, C < B < B+ < A < A+, mapped to an
     ordinal 1-5 in both views (RATING_MAP). "P" (score pending) and "" (no
     rating) become <NA>, dropped by the standard pipelines and kept by the
     complete ones.

  2. EXEMPT PROVIDERS. Providers with `Permit Type == "Not Licensed (Exempt)"`
     carry no permit number but are rated. synthesize_exempt_ids() mints a
     stable surrogate key, EXEMPT-<zip5>-<name-slug>.

  3. TARGET LEAKAGE IN `Facility Type`. For non-exempt rows it is a genuine
     facility code:
     ...
     For exempt rows it is a verbatim re-encoding of the rating:
         EAA -> A+     EXA -> A      EBB -> B+
         EXB -> B      EXT -> C      PEN -> P (pending)
     recode_facility_type() collapses every exempt code to a single "EXEMPT"
     category and asserts the leak is gone.

Everything else is a straight tabulation.
"""
import json
import re
...
# C is the entry rating (score 50-70) and A+ the top (86-100).
RATING_MAP = {
    'C': 1,
    'B': 2,
    'B+': 3,
...
VALID_RATINGS = tuple(sorted(RATING_MAP.values()))

EXEMPT_PERMIT_TYPE = 'Not Licensed (Exempt)'

# Facility codes seen only on exempt rows; each encodes the rating, not a type.
LEAKY_EXEMPT_FACILITY_CODES = {'EAA', 'EXA', 'EBB', 'EXB', 'EXT', 'PEN'}
EXEMPT_FACILITY_CODE = 'EXEMPT'

# Inferred from the observed capacity bands and the site's advanced search.
...
# `County` (oddly title-cased, e.g. "Mccormick") is dropped in favour of
# `source_county`, which carries the canonical spelling.
SOURCE_RENAME = {
...
# abc_level is already the 1-5 ordinal (map_rating) by the time finalize() runs.
TARGET_RENAME = {'abc_level': 'qr_rating'}

# Identity/contact text, the duplicate county and constant state columns, the
# `exempt` flag (collinear with facility_type_code == "EXEMPT"), and
# facility_type_code once it has been one-hot encoded or labelled.
NON_FEATURE_COLS = [
    'provider_name',
    'operator',
    'phone',
...
# Target leakage: no feature may be derived from the rating or its
# administrative lifecycle (award / expiration / renewal dates, rating status).
# `Last ABC Inspection Date` is an ABC Quality rating-review visit, not a DSS
# licensing inspection: presence of a date tracks rating status, and recency
# tracks the re-review cycle, so it and its derivatives are banned too.
LEAKAGE_COLS = [
    'rating_status',
    'rating_pending',
...
# Derived forms of a leakage field (one-hot, ordinal, missingness, bin).
LEAKAGE_PREFIXES = (
...
def drop_leakage_columns(df, log=None):
    """Remove every rating-derived / rating-lifecycle column except the target."""
...
def normalize_source_columns(df):
    ...
    # permit_number must stay a string ('16941' must not become '16941.0').
    ...
    df['capacity'] = null_exempt_capacity(df)
    df['zip'] = df['zip'].map(normalize_zip)
    return df


def null_exempt_capacity(df):
    """Blank out the `Capacity = 0` placeholder on exempt providers.

    Exempt providers are unlicensed, so the state records no capacity and the
    export writes a literal 0; <NA> is what the cell means. Keyed on
    `permit_type` because the exempt facility codes are not collapsed yet.
    A non-exempt 0 is left alone."""
    exempt = df['permit_type'].map(_clean_str).eq(EXEMPT_PERMIT_TYPE)
    placeholder = exempt & df['capacity'].eq(0).fillna(False)
    return df['capacity'].mask(placeholder.to_numpy(dtype=bool), pd.NA)
...
def normalize_zip(value):
    """Keep the 5-digit ZIP (the source has a ZIP+4)."""
    s = _clean_str(value)
    m = re.match(r'(\d{5})', s)
...
def synthesize_exempt_ids(df, log=None):
    ...
    Without it a dedup on the blank permit_number would collapse them into one
    row. The prefix keeps it from colliding with a numeric permit number."""
...
def recode_facility_type(df, log=None):
    ...
    then assert that no surviving code determines the rating."""
...
def _assert_no_rating_leak(df):
    """Fail loudly if any facility code (on >1 rated row) maps to one rating."""
