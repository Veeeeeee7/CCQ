...
Montana's records are flatter than WI's: there are no delimited multi-value
cells and no nested JSON blobs, because the two source systems are a static
5-column PDF (ratings) and a licensing search whose payload is already one flat
object per provider. So the decomposition work here is different in kind:

  * the categorical columns (program_type, provider_type, county, ccrr_region)
    have their value space DISCOVERED from the data and one-hotted in the full
    set, and are kept as readable text in the raw set;

  * program_name is free text with no delimiter, so -- exactly as WI does for
    pr_program_philosophy -- it is decomposed against a keyterm vocabulary
    rather than split. The vocabulary below is seeded from the values actually
    observed in the STARS list and is meant to be edited.

Two MT-specific quirks the pipelines have to absorb:
  ...
  1. qr_rating's "Pre-Star" level means "accepted into STARS but not yet
     ...
     valid_target_values=(1,2,3,4,5); the complete scripts keep it. In the raw
     set it survives as the literal text "Pre-Star"; in the full set the target
     is coerced to numeric first, so it becomes NaN and the full outputs stay
     entirely numeric/boolean.

  2. provider_id is a real MAQCS license number ("PV109509") for programs the
     crawler could tie to a live license, and a synthetic "MT-CITY-NAME" key for
     programs that closed or were renamed between the frozen 7/1/2023 rating
     snapshot and today's licensing directory. `provider_id_source` records
     which; the full set exposes the same fact as the boolean `has_license`.
"""
import json
import re
...
# program_name has no delimiter, so its atomic vocabulary cannot be inferred by
# splitting. Seeded from the program names observed in the STARS PDF. Edit
# freely. Note "Head Start" also fires inside "Early Head Start", which is
# intended -- an EHS program is a Head Start program.
NAME_KEYTERMS = [
    "Head Start",
    "Early Head Start",
    "Montessori",
...
# Identifier handling. provider_number is the unique grain (one row per rated
# STARS program); it is renamed to provider_id to match the downstream
...
# (utils.TARGET_COL == "qr_rating"). star_level is the native STARS column.
TARGET_RENAME = {"star_level": "qr_rating"}

# Columns excluded from the modelling feature set: join diagnostics, redundant
# identifiers, and free-text contact/address metadata that should not be fed to
# a model as features. provider_id itself is retained (the experiment loader
# drops it); everything listed here is removed from the output entirely.
# program_name is NOT listed: it is genuinely useful text for the raw set, and
# is excluded from the full set simply by being absent from that scaffold.
NON_FEATURE_COLS = [
    "sf_id",
    "provider_name",
    "street",
...
# Columns every clean script needs mt_records.csv to have. `provider_id_source`
# and the synthetic-id scheme were added to mt_crawler.py after the first full
# run, so a records file left over from that run is missing it -- which used to
# surface as a bare KeyError deep inside add_has_license(). Fail loudly and
# actionably instead.
...
def read_records(path):
    """Load mt_records.csv with the invariants every clean script relies on.

    dtype=str is not optional: provider_id must never be coerced (real ones look
    like "PV109509", synthetic ones like "MT-BILLINGS-TLC-DAYCARE"), and
    zip_code must keep any leading zero.
    """
    ...
    if missing:
        raise SystemExit(
            ...
            f'This almost always means the file is STALE -- written by a '
            f'crawler run predating the synthetic-provider_id change.\n'
            f'Regenerate it:\n'
            f'    python mt_crawler.py --join-only\n')
...
def build_keyterm_columns(df, column, keyterms, prefix, as_bool):
    """For free-text fields with no usable delimiter (here: program_name), flag
    each keyterm found in the cell. Matching is token-boundary aware on the
    slugged text, so 'Learning Center' matches '..._learning_center_...' and
    never fires on a substring like 'learning_centered'.
...
def coerce_star_numeric(df, column='star_level'):
    """Full set only. "Pre-Star" -> NaN, "1".."5" -> ints, so the target column
    is numeric before finalize() sees it. The standard scripts would coerce
    anyway inside finalize(); doing it here is what keeps the COMPLETE full
    output numeric too, since that path passes no valid_target_values."""
...
def add_has_license(df):
    """Boolean twin of provider_id_source, for the numeric-only full set.
    True  -> provider_id is a real MAQCS license number.
    False -> provider_id is a synthetic MT-CITY-NAME key (program not found in
             the live licensing directory)."""
...
def strip_dollar_prefix(series):
    ...
    else is untouched. Kept for parity with the GA/WI pipelines; no-op on MT."""
    def _clean(x):
...
def finalize(df, columns_file, which, key, target, dynamic_prefixes,
    ...
    valid_target_values: controls which rows survive on the target.
      - None (complete set): keep every row, including rows whose rating is
        invalid ("Pre-Star") or missing (NaN).
      - an iterable of allowed values (e.g. (1, 2, 3, 4, 5)): keep only rows
        ...
        "Pre-Star" and NaN in one step.
    """
    # Rename the grain to provider_id and the target to qr_rating, then drop
    # non-feature columns (join diagnostics / metadata) before assembling the
    # schema.
    ...
    # stale mt_columns.json still lists them.
